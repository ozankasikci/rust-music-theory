...
lazy_static! {
    static ref QUALITY_REGEXES: Vec<(Regex, Quality)> = {
        ...
        vec![
            (Regex::new(r"^(M\s+|M$|(?i)maj|Maj|Major|major)").unwrap(), Major),
            (Regex::new(r"^(m\s+|m$|(?i)min|Min|Minor|minor)").unwrap(), Minor),
            (Regex::new(r"(?i)^(diminished)").unwrap(), Diminished),
            (Regex::new(r"(?i)^(augmented)").unwrap(), Augmented),
            (Regex::new(r"(?i)^(half\s*diminished|halfdiminished)").unwrap(), HalfDiminished),
            (Regex::new(r"(?i)^(dominant)").unwrap(), Dominant),
            (Regex::new(r"(?i)^(sus2\s+|sus2$|suspended2)").unwrap(), Suspended2),
            (Regex::new(r"(?i)^(sus4\s+|sus4$|suspended4)").unwrap(), Suspended4),
        ]
    };
}
