...
use std::io::{Write, stdin, stdout, StdoutLock};
...
fn main() {
    ...
    print_color(&mut stdout, "\nEnter a number to select the scale type:".to_string(), color::Fg(color::Green));
    let mut index = 1;
    ...
    print_color(&mut stdout, "\nEnter a number to select the mode:".to_string(), color::Fg(color::Green));
    let mut index = 1;
    ...
    print_color(&mut stdout, "\nEnter a number to select the pitch:".to_string(), color::Fg(color::Green));
    let mut index = 1;
    ...
    print_color(&mut stdout, format!("\nThe notes for {} {} {} scale", pitch_choice, scale_choice, scale.mode.unwrap()), color::Fg(color::Green));
    for (i, note) in notes.iter().enumerate() {
        print!("{}-",i + 1);
        print_color(&mut stdout, format!("{}  ", note.pitch_class), color::Fg(color::Red));
    }
    println!("");
    println!("");
}
